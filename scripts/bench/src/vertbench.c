// vertbench - Vulkan VERTEX-bound microbenchmark for the Adreno GPU.
// Draws a large vertex count into a tiny (32x32) render target so the fragment/raster
// stage is negligible and per-vertex transform work dominates. The vertex shader runs
// a heavy transform loop (iters). Reports vertices/s and nominal vertex GFLOPS.
// Purpose: measure the effect of GAMMA_FP16_VERTEX (fp16 vertex math) which the
// fragment-bound benches (gamebench/gfxbench) cannot show. Args: VCOUNT iters reps.
...
int main(int argc,char**argv){
  uint32_t VCOUNT = argc>1?(uint32_t)atoi(argv[1]):3000000;
  int32_t  iters  = argc>2?atoi(argv[2]):64;
  ...
  // ~14 flops per loop iter (normalize+3 fma+2 trig approx), plus small fixed setup
  double flops=(double)VCOUNT*(double)iters*14.0;
  printf("vertbench verts=%u iters=%d best_s=%.5f mverts_per_s=%.1f gflops=%.1f\n",
         VCOUNT,iters,best,(double)VCOUNT/best/1e6,flops/best/1e9);
  return 0;
}
