// drawbench - Vulkan CPU-SUBMISSION-bound microbenchmark for the Turnip driver.
// Re-records a command buffer of DRAWS tiny draws per frame (each preceded by a
// push-constant change so per-draw state is emitted), into a tiny 32x32 render
// target with a trivial 3-vertex triangle and a near-empty shader. GPU work is
// negligible, so the measured wall-clock is dominated by the DRIVER's per-draw
// CPU cost (command recording + state/descriptor emission). This is the signal
// lever #4 (draw/command-submission CPU-overhead reduction) needs - the existing
// GPU-throughput benches cannot show it. Reports draws/s and us-per-draw.
// Governor-independent: pure CPU wall-clock of the recording path.
// Args: DRAWS reps. Reuses vertbench's trivial vert/frag SPIR-V (iters push=1).
...
int main(int argc,char**argv){
  ...
  // Warm up once (first record triggers pipeline bind lowering etc.).
  double best=1e18;
  for(uint32_t r=0;r<reps+1;r++){
    ...
    double dt=now()-t0;
    if(r>0 && dt<best) best=dt;                      // skip warm-up rep 0
  }
  printf("drawbench draws=%u best_s=%.5f draws_per_s=%.0f us_per_draw=%.3f\n",
