...
| `GammaOS-Turnip-Adreno613-GameNative-ubwc-cfp16-v1.adpkg.zip` | ubwc + compute fp16 | **Emulator, compute-heavy titles (opt-in, title-dependent).** ubwc-v6 plus forced fp16 on compute shaders (measured +124% on the compute microbench). Device-validated RENDERS CLEAN on MGS4/RPCS3 with a small gain, but forced compute fp16 BLACK-SCREENS some content (e.g. 3DMark Wild Life Extreme), so it is per-title opt-in, not a default - try it on a compute-heavy title and fall back to plain ubwc if anything corrupts. |
