...
|----------------|-------|-------|
| [GammaOS-Turnip-Adreno613-GameNative-ubwc-nofp16-v1.adpkg.zip](GammaOS-Turnip-Adreno613-GameNative-ubwc-nofp16-v1.adpkg.zip) | ubwc, no forced fp16 | **RECOMMENDED default for GameNative / Winlator emulation - import this one.** UBWC + direct-to-sysmem (the +43% multi-pass render-to-texture win, 5680 -> 8118 passes/s, plus coherency) with forced fragment fp16 OFF, so it renders EVERY tested title correctly - including Fox Engine (MGS V: Ground Zeroes), where forced fp16 blacks the materials. Device-validated clean on MGS4 and Ground Zeroes. Gives up only the fp16 fragment-ALU speedup, which is small-to-nil on CPU-bound titles and unusable on fp16-sensitive ones anyway. Import into the emulator's graphics-driver picker; do NOT flash as the system driver (the /vendor driver stays ULTRA-v6). |
