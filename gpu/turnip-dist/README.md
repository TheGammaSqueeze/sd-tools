...
|----------------|-------|-------|
| [GammaOS-Turnip-Adreno613-GameNative-ubwc-v1.adpkg.zip](GammaOS-Turnip-Adreno613-GameNative-ubwc-v1.adpkg.zip) | ULTRA-v6 + UBWC + sysmem | **RECOMMENDED for GameNative / Winlator emulation - import this one.** Device-validated on Metal Gear Solid 4 (PC port via GameNative): renders clean, no black textures, and noticeably faster than the community Turnip A12-fix. Re-enables UBWC framebuffer compression (lossless) plus direct-to-sysmem. Measured +43% on the multi-pass render-to-texture benchmark (5680 -> 8118 passes/s) modelling DXVK/VKD3D post-process and render-target chains (bloom/SSAO/tonemap). Import it into the emulator's graphics-driver picker; do NOT flash it as the system driver (UBWC+sysmem cost ~7% and more DRAM power on native content - the system driver stays ULTRA-v6). **If a title shows black textures on this driver, use ubwc-nofp16 below** (forced fp16 blacks textures in some engines, e.g. Fox Engine / MGS V: Ground Zeroes). |
| [GammaOS-Turnip-Adreno613-GameNative-ubwc-nofp16-v1.adpkg.zip](GammaOS-Turnip-Adreno613-GameNative-ubwc-nofp16-v1.adpkg.zip) | ubwc, no forced fp16 | **Alternative to ubwc - use when ubwc shows black textures.** Same UBWC + sysmem (keeps the +43% render-to-texture win and coherency), but with forced fragment fp16 turned OFF. Device-validated RENDERS CLEAN on MGS V: Ground Zeroes (Fox Engine), where plain ubwc's forced fp16 blacks textures because the selective protection misses that engine's texture-coordinate path. Loses the fp16 fragment-ALU speedup but is the safe choice for fp16-sensitive titles; equally fine on CPU-bound titles like MGS4 where fp16 does not matter anyway. |
