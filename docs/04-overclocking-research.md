...
- Therefore CPU OC requires editing the **firmware**: either the EPSS/CPRh LUT rows in `devcfg.mbn`, or the `tgt_cpucp_config` consumed by `cpucp.elf`. Both `devcfg.mbn` and `cpucp.elf` are signed images. Since this package is a `_debug` build and we can re-sign, the path is:
  1. Locate the APSS/EPSS LUT table inside `devcfg.mbn` near the `tgt_cpucp_config` string at 0x4C57.
     ...
     Measured note (do not assume kHz): a scan of the devcfg config segment and cpucp.elf for monotonic kHz-format u32 sequences in 300000..3500000 returned nothing. The EPSS LUT does not store frequencies as kHz. Each LUT row stores an `lval` (the target frequency expressed as a multiple of the source clock, roughly `freq / 19.2 MHz`, so ~30..170) together with the voltage-corner index, not a kHz value. The region around `tgt_cpucp_config` at 0x4C57 in devcfg is a property-name string table; the actual row data is elsewhere in the config segment (ph[1] at file offset 0x1000, size 0x7ee8) or is assembled by cpucp at runtime. cpucp.elf is ELF32 RISC-V, so pinpointing the row struct reliably needs a Ghidra RISC-V disassembly of cpucp cross-referenced against the SM6450 BSP; the `lval` encoding is why a naive kHz scan fails. This is the concrete next step and must be done before any CPU-OC edit, rather than guessing offsets.
