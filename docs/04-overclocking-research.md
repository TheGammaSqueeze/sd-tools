...
- Therefore CPU OC requires editing the **firmware**: either the EPSS/CPRh LUT rows in `devcfg.mbn`, or the `tgt_cpucp_config` consumed by `cpucp.elf`. Both `devcfg.mbn` and `cpucp.elf` are signed images. Since this package is a `_debug` build and we can re-sign, the path is:
  1. Locate the APSS/EPSS LUT table inside `devcfg.mbn` near the `tgt_cpucp_config` string at 0x4C57.
     ...
     The region around `tgt_cpucp_config` at 0x4C57 is a property-name string table. The row data is therefore inside cpucp.elf's data section (ELF32 RISC-V) or assembled by cpucp at runtime, not a flat table a byte-scan can find. The definitive next step is a Ghidra RISC-V disassembly of cpucp.elf cross-referenced against the SM6450 BSP to recover the row struct; only then edit and re-sign. Do not fabricate offsets. UPDATE: this disassembly was done (see docs/06). Result: cpucp.elf is a pure DCVS engine, it materializes the EPSS domain bases 0x17d90/91/92000 only for control/status registers (offsets 0x000/0x054/0x300/0x304/0x340/0x350-0x35c/0x3bc) and never touches the FREQ_LUT register window, and it carries no lval source array. So the CPU LUT is NOT in cpucp; it is devcfg/OSM data programmed into the EPSS hardware by XBL (xbl_s.melf, ARM64). The next target is the EPSS-LUT-writing code in xbl_s.melf and its devcfg source rows. FURTHER UPDATE (docs/07): the xbl_s.melf and uefi.elf executable segments that hold the OSM/EPSS bring-up are COMPRESSED (the ph0 loader payload and LZMA DXE sections), the EPSS bases never appear as literals in any uncompressed data, and devcfg/xbl_config are DALConfig name/value blobs with no plaintext OPP rows; the voltage side is CPRh/corner-composed at runtime. Conclusion (high confidence): there is NO editable static EPSS LUT in the shipped images, so CPU overclock by a static firmware byte-edit is not viable from these artifacts. The realistic CPU-OC routes are (a) decompress the xbl_s.melf loader payload, edit the freq-plan array inside it, and re-sign, or (b) read/write the live EPSS FREQ_LUT on a booted device (domain0 0x17d91000, domain1 0x17d92000). Any lval bump must stay within the CPRh voltage envelope. FINAL UPDATE (docs/07): the XBL Core UEFI FV in uefi.elf was decompressed (tools/fw/extract_uefi.sh) and its ClockDxe fully analyzed. The frequency-config structs it holds are GPU (gpu_cc_pll0/pll1) and display (disp_cc_pll0) PLL VCO tables, NOT CPU clocks; the real CPU clock descriptors (apcs_silver/gold/l3_post_acd_clk) point at zero-filled slots and no CPU frequency (2.0/2.2 GHz) exists anywhere in the image. This confirms with high confidence that the CPU operating-point plan is fetched at runtime from the EPSS/OSM hardware LUT (ClockApps_GetCPUFrequencyPlan), so there is no static array to patch in ANY shipped image. The only CPU-OC lever on this device is the live EPSS LUT, tools/fw/epss_lut.sh (domain0 0x17d91000, domain1 0x17d92000), within the CPRh envelope.
